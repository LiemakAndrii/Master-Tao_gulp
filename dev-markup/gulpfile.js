...
gulp.task("sass", function () {
	return gulp
		.src(dirs.source + "/style.scss") // какой файл компилировать (путь из константы)
		.pipe(include())
...
gulp.task("icons", function () {
	return gulp
		.src(
			[
				dirs.source + "/assets/icons/*.svg", // какие файлы обрабатывать (путь из константы, маска имени, много расширений)
			],
			{ since: gulp.lastRun("icons") } // оставим в потоке обработки только изменившиеся от последнего запуска задачи (в этой сессии) файлы
		)
		.pipe(plumber({ errorHandler: onError }))
		.pipe(newer(dirs.build + "/assets/icons")) // оставить в потоке только новые файлы (сравниваем с содержимым папки билда)
		.pipe(gulp.dest(dirs.build + "/assets/icons")); // записываем файлы (путь из константы)
});
...
gulp.task(
	...
	gulp.series("build", function () {
		...
		gulp.watch(
			// следим за SVG
			dirs.source + "/assets/img/icons/*.svg",
			gulp.series("svgstore", "html", 'icons', reloader)
		);
		gulp.watch(
